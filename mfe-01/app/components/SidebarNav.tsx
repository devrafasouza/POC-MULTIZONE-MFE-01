...
type SidebarNavProps = {
  basePath: string; // ex.: "/mfe-02"
  otherBasePath: string; // ex.: "/mfe-01"
};
...
export default function SidebarNav({
  ...
  // Use hrefs RELATIVOS aqui: o Next vai aplicar o basePath automaticamente no <Link>
  const items = [
    { label: 'Home', href: '/' }, // vira /mfe-xx/ na prática
    { label: 'Reset de Senha', href: '/reset' }, // vira /mfe-xx/reset
  ];

  // Para destacar item ativo, compare com o caminho ABSOLUTO (com basePath)
      ...
      <div className="sticky top-16 flex h-[calc(100vh-4rem)] flex-col gap-2 p-4">
        ...
        <nav className="flex flex-col gap-1">
          {items.map((it) => (
            <Link
              key={it.href}
              href={it.href} // <<< SEM basePath aqui
              className={cx(
          ...
          {/* cross-MFE: hard links, aqui você inclui o basePath do OUTRO MFE */}
          <a
            href={otherBasePath}
